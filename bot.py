...
async def process_sysrole(msg: Message, uid: int, priority: str):
    ...
    try:
        ...
        logger.error(f"ERROR in sysrole: {e}")
        traceback.print_exc()
...
async def addrole(msg: Message, priority: str, role_name: str):
    ...
    try:
        ...
        return await msg.answer(f"✅ РОЛЬ СОЗДАНА/ОБНОВЛЕНА\n\n📋 Имя: {role_name}\n📊 Приоритет: {priority_int}")
...
async def giverole_reply(msg: Message, priority: str):
    if not msg.reply_message:
        return await msg.answer("❌ Ответьте на сообщение пользователя")
...
async def giverole(msg: Message, user_info: str, priority: str):
    ...
    try:
        ...
        uid = extract(msg)
        if not uid:
            uid = await resolve_user_id(user_info.replace("@", ""))
...
async def process_giverole(msg: Message, uid: int, priority: str, sender_role: int):
    conn, cur = db()
    try:
        priority_int = int(priority)
        ...
        return await msg.answer("❌ Ошибка при выдаче роли")
    finally:
        conn.close()
...
async def list_roles(msg: Message):
    ...
    try:
        ...
        if not roles:
            return await msg.answer("📊 Нет созданных ролей\n\nСоздайте: /addrole 50 Модератор")
        
        text = "📊 СПИСОК РОЛЕЙ:\n\n"
        for priority, name in roles:
            text += f"📋 {priority} - {name}\n"
        
        return await msg.answer(text)
    
    except Exception as e:
        logger.error(f"ERROR in list_roles: {e}")
        return await msg.answer("❌ Ошибка при получении ролей")
    finally:
        conn.close()
...
async def staff(msg: Message):
    ...
    try:
        ...
        if not staff_list:
            return await msg.answer("👥 Нет пользователей с ролями")
        ...
        for user_id, role_priority in staff_list:
            ...
            status = "✅"
            if is_user_banned(msg.peer_id, user_id):
                status = "🚫"
            elif is_user_muted(msg.peer_id, user_id):
                status = "🔇"
            
            text += f"{status} {user_name} (роль: {role_priority})\n"
        
        return await msg.answer(text)
    
    except Exception as e:
        logger.error(f"ERROR in staff: {e}")
        return await msg.answer("❌ Ошибка при получении списка")
    finally:
        conn.close()
...
async def warn_reply(msg: Message, reason: str = "Без причины"):
    if not msg.reply_message:
        return await msg.answer("❌ Ответьте на сообщение пользователя")
    ...
    try:
        ...
        await process_warn(msg, msg.reply_message.from_id, reason, user_role)
    finally:
        conn.close()
...
async def process_warn(msg: Message, uid: int, reason: str, sender_role: int):
    ...
    try:
        ...
        if warns >= 3:
            ...
            VALUES (%s, %s, 'ban', 'Автоматический бан (3 предупреждения)')
            """, (uid, msg.peer_id))
...
async def mute_help(msg: Message):
    return await msg.answer(
        ...
        "или ответьте на сообщение: /mute [время]\n\n"
        "Форматы: 10m, 1h, 1d"
    )
...
async def mute_reply(msg: Message, time_or_reason: str):
    ...
    try:
        ...
        await process_mute(msg, msg.reply_message.from_id, time_or_reason, "")
    finally:
        conn.close()
...
async def mute(msg: Message, user_info: str, time_or_reason: str):
    ...
    try:
        ...
        await process_mute(msg, uid, time_or_reason, "")
    finally:
        conn.close()

async def process_mute(msg: Message, uid: int, time_or_reason: str, reason: str):
    conn, cur = db()
    try:
        duration = None
        ...
        VALUES (%s, %s, 'mute', %s, %s)
        """, (uid, msg.peer_id, end_time, final_reason))
...
async def unmute_help(msg: Message):
    return await msg.answer("🔊 КОМАНДА: СНЯТЬ МУТ\n\n/unmute @пользователь\nили ответьте на сообщение: /unmute")
...
async def unmute_reply(msg: Message):
    ...
    conn, cur = db()
    try:
        cur.execute("DELETE FROM punishments WHERE user_id=%s AND peer_id=%s AND type='mute'",
                   (msg.reply_message.from_id, msg.peer_id))
...
async def ban_help(msg: Message):
    return await msg.answer(
        ...
        "или ответьте на сообщение: /ban [время]\n\n"
...
async def ban_reply(msg: Message, time_or_reason: str):
    ...
    try:
        ...
        await process_ban(msg, msg.reply_message.from_id, time_or_reason, "")
    finally:
        conn.close()
...
async def ban(msg: Message, user_info: str, time_or_reason: str):
    ...
    try:
        ...
        await process_ban(msg, uid, time_or_reason, "")
    finally:
        conn.close()

async def process_ban(msg: Message, uid: int, time_or_reason: str, reason: str):
    conn, cur = db()
    try:
        duration = None
        final_reason = reason if reason else "Без причины"
...
async def unban_help(msg: Message):
    return await msg.answer("✅ КОМАНДА: РАЗБАНИТЬ\n\n/unban @пользователь\nили ответьте на сообщение: /unban")
...
async def unban_reply(msg: Message):
    ...
    conn, cur = db()
    try:
        cur.execute("DELETE FROM punishments WHERE user_id=%s AND peer_id=%s AND type='ban'",
                   (msg.reply_message.from_id, msg.peer_id))
...
async def kick_help(msg: Message):
    return await msg.answer("👢 КОМАНДА: ИСКЛЮЧЕНИЕ\n\n/kick @пользователь\nили ответьте на сообщение: /kick")
...
async def kick_reply(msg: Message):
    ...
    try:
        ...
        await kick_user(msg.peer_id, msg.reply_message.from_id)
...
async def snick_help(msg: Message):
    return await msg.answer("🏷 КОМАНДА: НИК\n\n/snick [ник]\nМаксимум 50 символов")
...
async def snick(msg: Message, nick: str):
    ...
    try:
        ...
        """, (msg.from_id, msg.peer_id, nick, nick))
        
        await msg.answer(f"✅ Ник установлен: {nick}")
    finally:
        conn.close()
...
async def rnick_help(msg: Message):
    return await msg.answer("🧹 КОМАНДА: УДАЛИТЬ НИК\n\n/rnick")

@bot.on.message(text="/rnick")
async def rnick(msg: Message):
    conn, cur = db()
    try:
        cur.execute("UPDATE users SET nickname=NULL WHERE user_id=%s AND peer_id=%s",
...
async def stats_help(msg: Message):
    return await msg.answer("📊 КОМАНДА: СТАТИСТИКА\n\n/stats @пользователь\nили ответьте на сообщение: /stats")
...
async def process_stats(msg: Message, uid: int):
    conn, cur = db()
    try:
        cur.execute("SELECT role, msgs, warn_count, nickname FROM users WHERE user_id=%s AND peer_id=%s",
                   (uid, msg.peer_id))
        res = cur.fetchone()
        ...
        status = "✅"
        if is_user_banned(msg.peer_id, uid):
            status = "🚫 ЗАБАНЕН"
        elif is_user_muted(msg.peer_id, uid):
            status = "🔇 ЗАМУЧЕН"
                ...
                for user_id, peer_id in expired_bans:
                    ...
                    try:
                        await bot.api.messages.send(
                            peer_id=peer_id,
                            message=f"✅ СРОК БАНА ИСТЁК\n\n👤 @id{user_id} может снова присоединиться",
                            random_id=0
                        )
                    except:
                        pass
