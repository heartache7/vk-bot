...
async def sysrole_help(msg: Message):
    ...
    return await msg.answer(
        ...
        "/sysrole @пользователь [приоритет] - выдать роль в текущей беседе\n\n"
        "⚙️ Примеры:\n"
        "• /sysrole info\n"
        "• /sysrole @Ivan 50\n"
        "• /sysrole @Maria 100"
...
async def sysrole_info(msg: Message):
    ...
    return await msg.answer(
        ...
        "🔧 УПРАВЛЕНИЕ РОЛЯМИ:\n"
        "• /sysrole @user [приоритет] - выдать роль в текущей беседе\n"
...
async def addrole(msg: Message, priority: str, role_name: str):
    ...
    try:
        ...
        default_role = get_default_cmd_role('addrole')
        if not check_cmd_permission(cur, msg.peer_id, 'addrole', sender_role_val):
...
async def stats_help(msg: Message):
    return await msg.answer(
        ...
        "⚙️ Примеры:\n"
        "• /stats @Ivan - информация о пользователе\n"
        "• /stats - ваша статистика\n"
        "• (ответить на сообщение) /stats"
    )
    ...
    try:
        ...
        if not res:
            return await msg.answer(
                f"📊 СТАТИСТИКА\n\n"
                f"👤 {user_name} (id{uid})\n\n"
                f"❌ Нет данных в системе\n"
                f"(пользователь ещё не активен в чате)"
            )
